...
class Engine(object):
    ...
    def __init__(self, datamanager, model_1,model_2, optimizer=None, scheduler=None, use_gpu=True):
    ...
    def test(self, epoch, valloader, testloader, testdataset, dist_metric='euclidean', normalize_feature=False,
        ...
        if testloader is not None:
            ...
            qf_1, gf_1, q_pids, g_pids=self._evaluate_1(
                epoch,
            ...
            qf_2, gf_2, _, _=self._evaluate_2(
                epoch,
                queryloader=queryloader,
                galleryloader=galleryloader,
                testdataset=testdataset,
                ...
                rerank=rerank,
                return_json=True)
            print('finish qf_1 and qf2')

            self._evaluate_last(
                qf_1, gf_1,qf_2, gf_2, q_pids, g_pids,
                epoch,
                queryloader=queryloader,
                galleryloader=galleryloader,
                testdataset=testdataset,
                ...
                rerank=rerank,
                return_json=True)
            print('finish qf_1 and qf2 and json')
    ...
    @torch.no_grad()
    def _evaluate_1(self, epoch, queryloader=None, galleryloader=None, testdataset=None,
                  dist_metric='euclidean', normalize_feature=False, save_dir='', use_metric_cuhk03=False,
                  ranks=[1, 5, 10, 20],
                  rerank=False, multi_scale_interpolate_mode='bilinear', multi_scale=(1,),
                  return_json=False):  # multi_scale=(1, 1.1, 1.2,)
    ...
    @torch.no_grad()
    def _evaluate_2(self, epoch, queryloader=None, galleryloader=None, testdataset=None,
                  dist_metric='euclidean', normalize_feature=False, save_dir='', use_metric_cuhk03=False,
                  ranks=[1, 5, 10, 20],
                  rerank=False, multi_scale_interpolate_mode='bilinear', multi_scale=(1,),
                  return_json=False):  # multi_scale=(1, 1.1, 1.2,)
        ...
        if self.use_gpu:
            imgs = imgs.cuda()
        feature_test = self._extract_features_2(imgs)  # bs not suitable for last batch size
        qf, q_pids = [], []  # query features, query person IDs
        for batch_idx, data in enumerate(queryloader):
            imgs, pids = self._parse_data_for_eval(data)
            end = time.time()
    ...
    @torch.no_grad()
    def _evaluate_last(self,qf_1, gf_1,qf_2, gf_2, q_pids, g_pids, epoch, queryloader=None, galleryloader=None, testdataset=None,
                  dist_metric='euclidean', normalize_feature=False, save_dir='', use_metric_cuhk03=False,
                  ranks=[1, 5, 10, 20],
                  rerank=False, multi_scale_interpolate_mode='bilinear', multi_scale=(1,),
                  return_json=False):  # multi_scale=(1, 1.1, 1.2,)
        ...
        qf_1, gf_1, qf_2, gf_2, q_pids, g_pids =qf_1, gf_1,qf_2, gf_2, q_pids, g_pids
