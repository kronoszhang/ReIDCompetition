...
__all__ = ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'resnext50_32x4d',           'pcb',
           'resnext101_32x8d', 'resnet50_fc512',
           'resnet50_ibn_a', 'resnet50_ibn_old_a', 'resnet50_ibn_b',
           'resnet101_ibn_a', 'resnet101_ibn_old_a', 'resnet101_ibn_b',
           'resnet152_ibn_a', 'resnet152_ibn_old_a', 'resnet152_ibn_b',
           'resnet50_ibn_a_fc512', 'resnet50_ibn_old_a_fc512', 'resnet50_ibn_b_fc512',
           'resnext50_ibn_a_32x4d', 'resnext101_ibn_a_32x8d',]
...
class Bottleneck_IBN_A(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 groups=1, base_width=64, dilation=1, norm_layer=None, ibn=True):
        super(Bottleneck_IBN_A, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
...
class IBN_OLD_A(nn.Module):
    def __init__(self, planes):
        super(IBN_OLD_A, self).__init__()
        half1 = int(planes/2)
        self.half = half1
        half2 = planes - half1
        self.IN = nn.InstanceNorm2d(half1, affine=True)
...
class Bottleneck_IBN_OLD_A(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 groups=1, base_width=64, dilation=1, norm_layer=None, ibn=True):
        super(Bottleneck_IBN_OLD_A, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
...
class ResNet_IBN_B(nn.Module):
    def __init__(self, num_classes, loss, block, layers, zero_init_residual=False,
                 last_stride=2, fc_dims=None, dropout_p=None,
                 bnneck=True, bnneck_test='after', **kwargs):
        scale = 64
        self.inplanes = scale
        self.loss = loss
        ...
        self.layer3 = self._make_layer(block, scale * 4, layers[2], stride=2, IN=False)
        self.layer4 = self._make_layer(block, scale * 8, layers[3], stride=last_stride, IN=False)
        self.global_avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.bnneck_layer = self._construct_bnneck_layer(bnneck, bnneck_test, self.feature_dim)
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck) or isinstance(m, Bottleneck_IBN_A) or isinstance(m, Bottleneck_IBN_OLD_A) \
                        or isinstance(m, Bottleneck_IBN_B):
...
def init_pretrained_weights(model, model_url):
    ...
    # find same layers in `pretrain_dict` and `model_dict`
    pretrain_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict and model_dict[k].size() == v.size()}
    # update `model_dict`, raw pretrained layer and new added layer params are all loaded
    model_dict.update(pretrain_dict)
    # load `model_dict`
    model.load_state_dict(model_dict)
    # model.load_state_dict(model_zoo.load_url(model_urls['resnet50'])) # directly load would lead layers mis-match
...
def resnet50(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        ...
        fc_dims=None,
        dropout_p=None,
        bnneck=True,  # default=False
        bnneck_test='after',   # default='fc'
        **kwargs
    )
    if pretrained:
...
def resnext50_ibn_a_32x4d(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck_IBN_A,
        layers=[3, 4, 6, 3],
        last_stride=2,
        fc_dims=None,
        dropout_p=None,
...
def resnext101_ibn_a_32x8d(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck_IBN_A,
...
def resnet152_ibn_a(num_classes, loss = 'softmax', pretrained=False, **kwargs):
...
def resnet50_ibn_a_fc512(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck_IBN_A,
...
IBN_OLD_A
"""
def resnet50_ibn_old_a(num_classes, loss = 'softmax', pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
...
def resnet101_ibn_old_a(num_classes, loss = 'softmax', pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
...
def resnet152_ibn_old_a(num_classes, loss = 'softmax', pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.
...
def resnet50_ibn_old_a_fc512(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck_IBN_OLD_A,
...
def resnet101_ibn_b(num_classes, loss = 'softmax', pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
...
def resnet152_ibn_b(num_classes, loss = 'softmax', pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.
...
def resnet50_ibn_b_fc512(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet_IBN_B(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck_IBN_B,
...
class PCB(nn.Module):
    ...
    def __init__(self, num_classes, loss, block, layers, zero_init_residual=False,
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=last_stride,
                                       dilate=replace_stride_with_dilation[2])
        self.global_avgpool = nn.AdaptiveAvgPool2d((self.part, 1))
        self.bnneck_layer = self._construct_bnneck_layer(bnneck, bnneck_test, self.feature_dim)
        self.fc = self._construct_fc_layer(fc_dims, 512 * block.expansion, dropout_p)
        #self.classifier = nn.Linear(self.feature_dim, num_classes)
        self._init_params()
    ...
    def forward(self, x, return_featuremaps=False):
        ...
        v1 = v.view(v.size(0), -1)   # [bs, 2048*6]

        if self.bnneck_layer is not None:
            v2 = self.bnneck_layer(v1)
...
def pcb(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = PCB(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck,
